...
> **本章是 `static` 与类初始化的唯一权威页。** [类初始化生命周期图](../03-图示/类初始化/类初始化流程.svg)对应本章主线；`final` 常量变量的引用只保留边界，详细语义回到 [final与常量设计.md](final与常量设计.md)。

配套验证：[ClassInitializationDemo.java](../04-示例代码/src/main/java/com/xuegucheng/javatechreview/core/ClassInitializationDemo.java) 与 [ClassInitializationDemoTest.java](../04-示例代码/src/test/java/com/xuegucheng/javatechreview/core/ClassInitializationDemoTest.java)。面试入口见 [类初始化与 final](../01-面试速记/类初始化与final.md)。
