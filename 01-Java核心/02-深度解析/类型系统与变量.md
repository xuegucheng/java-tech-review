...
- 自动宽化、强制窄化和数值提升：见 `类型转换与数值精度.md`；
- 运算符求值规则：见 `运算符与表达式陷阱.md`；
- 参数传递：见 `参数传递机制.md`；
- 对象创建和不可变设计：见 `对象创建与不可变设计.md`；
- `final` 与编译期常量：见 `final与常量设计.md`；
- `equals()` 与引用相等：见 `equals与hashCode契约.md`；
...
整数溢出和 `Math.addExact()` 等检查方法放在 `类型转换与数值精度.md`。
...
精度比较、`NaN`、舍入和金额计算见 `类型转换与数值精度.md`。
...
方法调用时，实参值会按值传给参数。完整参数传递规则见 `参数传递机制.md`。
