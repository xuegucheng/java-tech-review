...
- `System.identityHashCode` 解决什么问题，为什么不是务务 ID？
- `Objects` 的 null、比较、散列和索引检查方法如何使用？
- 订单、资源日志、对象复制、插件类型与资源对象应如何落地这些规则？
...
这些方法存在于所有对象，并不意味着所有务务类型都应该重写或直接使用它们。
...
默认输出只显示类型和哈希相关值，通常无法支持务务定位。良好实现应包含少量稳定识别字段：
...
`toString` 可能被日志框架、调试器、异常格式化和 IDE 自动调用。它不应修改状态、消费流、推进迭代器、触发重试、发送请求或抛务务异常。
...
实体 `toString` 访问懒加载关联可能触发数据存查询，脱离会话时还可能抛异常。ORM 实体应只输出本地已知的稳定标识和简单状态，避免遍历关联集合。
...
前者对 null 返回 `"null"`，后者允许自定义 null 默认文本。它们适合展示层或日志辅助，不应替代务务缺失值建模。
...
不要把类名字符串当作稳定务务协议，重命名和类加载器差异都会影响它。

## 19.23 代理与运行时类型

动态代理、字节码增强和 ORM 代理会让 `getClass()` 返回生成的代理类型，而不是源码中预期的务务类。精确类型判断、缓存键和日志应考虑代理解包或面向接口判断。
...
文件、Socket、数据存连接、线程、锁和事务上下文通常没有合理的 clone 语义。复制这些包装对象并不会复制底层操作系统资源，容易导致双重关闭和所有权混乱。
...
文件描述符、数据存连接、Socket 和锁需要确定性释放；GC 只管理 Java 堆对象可达性，不保证及时触发底层资源关闭。
...
复杂组件应提供 `start`/`close` 或由容器管理生命周期，并明确资源所有权。兜底清理只负责防止永久泄漏，不应承担正常务务流程。
...
`Objects.deepEquals(a,b)` 对两个数组使用深层比较语义，对非数组对象调用 equals。它适合通用容器工具，但务务类型最好明确知道字段类型并使用对应比较方法。
...
务务对象 `toString` 建议输出：对象 ID、状态、明细数量和 trace 标识；不要输出完整地址、手机号、Token 或全部明细。

稳定审计日志应使用结构化事件 DTO，而不是依赖对象 `toString`。

## 19.51 可选工程案例：不可变快照复制

存存、配置或资源快照应显式复制不可变值：

```java
record ResourceSnapshot(ResourceKey key, long amount, Instant capturedAt) { }
```

不要 clone 持有数据存会话、锁或懒加载集合的实体对象。
...
`Object.toString` 默认返回类名、@ 和当前 hashCode 的十六进制形式，所以它不是对象地址，也会受重写 hashCode 影响。务务类应提供简洁、无副作用、不会泄露敏感信息的 toString，但不能把它当序列化、缓存 Key 或签名协议。`getClass` 返回运行时类型且不能重写，代理场景下可能返回生成子类。`Object.clone` 是 protected，依赖 Cloneable 标记，并按字段赋值执行浅复制，引用成员默认共享且构造器不按普通方式执行，所以现代务务代码通常更推荐拷贝构造器、copyOf 工厂、with 方法或显式映射。wait/notify 定义在 Object 是因为任何对象都能作为监视器，调用前必须持有该对象锁，wait 应放在 while 条件循环中。finalize 已弃用并准备移除，资源释放应使用 try-with-resources 和显式 close，Cleaner 只能兜底。System.identityHashCode 用于诊断对象身份，不是地址或务务 ID；Objects 工具类可以简化 null、深比较和索引检查，但不能替代领域校验。
