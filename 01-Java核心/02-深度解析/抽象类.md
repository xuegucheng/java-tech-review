...
> **本页唯一负责抽象类/模板方法的语言规则。** LSP、组合取舍和接口隔离的系统推导统一放在 [组合与设计原则.md](组合与设计原则.md)；本页只说明抽象骨架如何落实父类契约。
...
- 继承、方法重写、字段隐藏和构造顺序基础：见 `继承与方法重写.md`；
- 编译时类型、运行时类型与动态绑定：见 `多态与动态绑定.md`；
- 接口语法、默认方法和多实现：见 `接口.md`；
- 抽象类、接口、组合、策略与设计原则的系统取舍：见 `组合与设计原则.md`；
...
```java
ReportTask task = new SummaryReportTask();
```

运行时对象中仍然包含父类部分：

```text
SummaryReportTask 对象
├── ReportTask 定义的状态
├── ReportTask 提供的实现
└── SummaryReportTask 补充的状态和实现
```
...
```java
public final class SummaryReportTask
        extends ReportTask {
...
public final class SummaryReportTask
    ...
    public SummaryReportTask(String taskId) {
        super(taskId);
    }
}
```
...
LSP 的完整判断表、组合替代和契约测试策略由 [组合与设计原则](组合与设计原则.md) 唯一维护；本页只说明抽象类如何把契约落实到骨架和扩展点。
...
```java
public abstract class AbstractCommandHandler<C> {
    ...
    public final CommandResult handle(C command) {
        String businessKey = businessKey(command);
        idempotency.ensureNotProcessed(businessKey);
        validate(command);

        try {
            List<CommandLine> lines = loadLines(command);
            CommandResult result = process(command, lines);
    ...
    protected abstract List<CommandLine> loadLines(
            C command
    );

    protected abstract CommandResult process(
            C command,
    ...
    protected abstract CommandCompletedEvent
            toCompletedEvent(
                    C command,
                    CommandResult result
            );
...
如果各操作类型在状态机、领域模型和一致性边界上完全不同，强行放入同一模板会形成大量空钩子和条件分支。
