...
> **本章定位：表达式规则收口页。** 只保留会影响结果、控制流或代码审查的运算符语义：数值提升、除法/溢出、引用相等、短路、位移、复合赋值、条件表达式和模式匹配。数值转换的完整规则见 [类型转换与数值精度.md](类型转换与数值精度.md)，业务相等性见 [equals与hashCode契约.md](equals与hashCode契约.md)。
...
对引用，`==` 只问两个引用值是否指向同一对象或同时为 `null`；它不是业务逻辑相等。字符串、值对象和领域对象的相等性见 [equals 与 hashCode 契约](equals与hashCode契约.md)。不要依赖字符串驻留或包装类型缓存让 `==` “偶尔正确”。
...
- 宽化、窄化、装箱和精度：见 [类型转换与数值精度.md](类型转换与数值精度.md)；
- 方法调用中的宽化/装箱/可变参数优先级：见 [方法定义与重载.md](方法定义与重载.md)；
- 对象身份与业务相等：见 [equals与hashCode契约.md](equals与hashCode契约.md)；
- `if`、`switch` 和循环结构：见 [流程控制.md](流程控制.md)。
