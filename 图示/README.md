...
- [重载解析](java/方法重载解析.svg)：严格调用、宽松调用、可变参数和最具体匹配；
- [Java 参数传递](java/Java值传递.svg)：引用值副本、对象修改与形参重绑；
- [多态分派](java/多态动态分派.svg)：编译时重载与运行时重写的两阶段关系；
- [类初始化](java/类初始化流程.svg)：加载、链接、准备和初始化；
- [final 引用与不可变对象](java/final引用与不可变对象.svg)：引用槽约束与对象状态约束。
