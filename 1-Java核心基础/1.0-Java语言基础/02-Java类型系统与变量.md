...
例如：
1 double value = 10; // 可以，int 自动提升为 double
2 int number = 10.5; // 不可以
3 int number = (int) 10.5; // 可以，但需要显式转换
需要注意：
“强类型”和“静态类型”不是同一个概念。
静态类型：类型检查主要发生在编译期。
...
1 String emoji = "😀";
2
3 System.out.println(emoji.length()); // 2
因此不能简单认为：
一个 char 永远等于一个人类可见字符。
更准确地说：
char 是一个 16 位 UTF-16 编码单元。
完整 Unicode 码点可通过：
1 int codePoint = emoji.codePointAt(0);
后面 String 章节再详细展开。
### 2.4.4 boolean 类型
...
字面量是直接写在代码中的值。
### 2.6.1 整数字面量
1 int decimal = 100; // 十进制
2 int binary = 0b1100100; // 二进制
3 int octal = 0144; // 八进制
4 int hex = 0x64; // 十六进制
结果都等于 100。
八进制字面量容易产生误解：
1 int value = 010;
...
### 2.6.2 数字下划线
1 int amount = 1_000_000;
2 long orderId = 2026_0721_001L;
下划线只用于提高可读性，不影响数值。
不能放在：
数字开头
数字结尾
...
1 public void execute() {
2 int count;
3 System.out.println(count); // 编译错误
4 }
### 2.7.2 方法参数
1 public void execute(int count) {
2 }
...
5 }
6
7 System.out.println(value); // 编译错误
