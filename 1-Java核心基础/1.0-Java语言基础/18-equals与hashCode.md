...
引用类型的 == 比较对象身份
↓
Object.equals 默认也是身份语义
↓
业务类型通过 equals 定义逻辑相等
↓
equals 必须满足五项契约
↓
重写 equals 时同步重写 hashCode
↓
哈希容器先按 hashCode 定位候选位置
↓
再通过 equals 确认逻辑相等
↓
相等性依赖字段必须稳定
...
Object 是 Java 普通类体系的根类。对于基本类型，`==` 比较数值；对于引用类型，`==` 比较两个引用是否指向同一个对象。Object 默认的 `equals()` 本质上也是身份比较，业务类型可以根据自身语义重写 `equals()` 来定义逻辑相等。

`equals()` 必须满足自反性、对称性、传递性、一致性和非空性。重写 `equals()` 后必须同步重写 `hashCode()`，并保证逻辑相等的对象拥有相同的哈希值。`HashMap` 和 `HashSet` 通常先通过 `hashCode()` 定位候选位置，再通过 `equals()` 确认逻辑相等。

参与 `equals()` 和 `hashCode()` 的字段应保持稳定，尤其不要随意修改已经作为 Map Key 的对象。实体通常使用稳定身份判断相等，值对象则通常按全部业务值判断相等。
