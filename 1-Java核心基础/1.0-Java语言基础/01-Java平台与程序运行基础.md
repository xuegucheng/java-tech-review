...
1 Java
2 ├── Java 语言
3 │ └── 语法、类型系统、面向对象、异常、泛型等
4 │
5 ├── Java 平台
6 │ └── JVM + 标准类库 + 工具链
7 │
8 └── Java 生态
9 └── Spring、Maven、Gradle、中间件及各种框架
需要明确：
Java 语言规定代码怎么写。
JVM 规定字节码怎么运行。
Java 标准类库提供常用 API。
...
3 同一套字节码
4 ↓
5 Windows JVM → Windows 机器指令
6 Linux JVM → Linux 机器指令
7 macOS JVM → macOS 机器指令
核心表达：
...
1 Java 版本：8 / 11 / 17 / 21
2 +
3 JDK 发行版：Oracle JDK / Temurin / Corretto / Zulu 等
...
对应类的完全限定名
例如：
1 com.example.demo.Main
### 1.10.2 classpath
classpath 是 JVM 和编译器寻找类的位置集合。
例如：
1 java -cp target/classes com.example.demo.Main
需要讲清：
