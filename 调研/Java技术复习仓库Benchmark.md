...
- Runnable 工程：没有 `示例代码/`、`pom.xml`、测试目录或可由仓库根目录执行的示例构建。
...
| B：全局按层 | `interview-review/`、`深度解析/`、`示例代码/` | 入口简单，快速层和深度层一眼可见 | 同名主题多，跨领域归属不清；新增内容容易堆成新的全局文件清单 | 不作为主架构 |
| C：技术域内部按层 | `01-Java核心/{面试速记,深度解析}`、`02-集合框架/{面试速记,深度解析}`，全局 `示例代码/`、`图示/` | 同时保留领域边界和认知层级；模块 README 可表达局部路线；示例与图可以复用 | 需要维护模块 README 和跨域链接 | **采用** |
...
- 迁移旧文章到按领域的 `深度解析/`，保持正文资产和 Git 历史；
