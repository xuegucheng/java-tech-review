...
> **本页唯一负责抽象类/模板方法的语言规则。** LSP、组合取舍和接口隔离的系统推导统一放在 [composition-and-design-principles.md](composition-and-design-principles.md)；本页只说明抽象骨架如何落实父类契约。
...
- 继承、方法重写、字段隐藏和构造顺序基础：见 `inheritance-and-overriding.md`；
- 编译时类型、运行时类型与动态绑定：见 `polymorphism-and-dynamic-dispatch.md`；
- 接口语法、默认方法和多实现：见 `interfaces.md`；
- 抽象类、接口、组合、策略与设计原则的系统取舍：见 `composition-and-design-principles.md`；
...
## 13.38 抽象类的继承契约：只保留模板侧边界

抽象子类不仅要补齐抽象方法，还必须保持父类的前置条件、后置条件、不变量、异常和关键副作用语义。模板入口可以用 `final` 固定流程，但不能用抽象类绕过行为契约。

LSP 的完整判断表、组合替代和契约测试策略由 [组合与设计原则](composition-and-design-principles.md) 唯一维护；本页只说明抽象类如何把契约落实到骨架和扩展点。
