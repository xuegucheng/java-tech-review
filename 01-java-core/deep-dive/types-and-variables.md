...
- 自动宽化、强制窄化和数值提升：见 `conversions-and-numeric-precision.md`；
- 运算符求值规则：见 `operators.md`；
- 参数传递：见 `parameter-passing.md`；
- 对象创建和不可变设计：见 `object-creation-and-immutability.md`；
- `final` 与编译期常量：见 `final-and-constants.md`；
- `equals()` 与引用相等：见 `equals-and-hashcode-contract.md`；
...
完整的多态和动态绑定规则见 `polymorphism-and-dynamic-dispatch.md`。
...
深拷贝和浅拷贝见 `parameter-passing.md` 与对象设计专题。
...
整数溢出和 `Math.addExact()` 等检查方法放在 `conversions-and-numeric-precision.md`。
...
精度比较、`NaN`、舍入和金额计算见 `conversions-and-numeric-precision.md`。
...
重载选择见 `methods-and-overloading.md`。
...
方法调用时，实参值会按值传给参数。完整参数传递规则见 `parameter-passing.md`。
