...
> **本章定位：语言规则权威页。** 只负责方法声明、方法签名、重载候选集、编译期解析、宽化/装箱/可变参数优先级、最具体匹配、`null` 歧义与泛型擦除。参数传递见 [parameter-passing.md](parameter-passing.md)，方法重写与动态分派见 [inheritance-and-overriding.md](inheritance-and-overriding.md) 和 [polymorphism-and-dynamic-dispatch.md](polymorphism-and-dynamic-dispatch.md)。
...
也可直接看 [重载解析图](../../diagrams/java/overload-resolution.svg)。图示只表达选择阶段，不替代 JLS 的完整方法调用转换规则。
...
接口之间也可能出现多个最具体默认候选；那是接口默认方法冲突，见 [interfaces.md](interfaces.md)，不要与普通重载歧义混为一谈。
...
子类新增的 `speak(String)` 不会让父类引用在运行时重新做一次重载匹配。完整动态分派链见 [polymorphism-and-dynamic-dispatch.md](polymorphism-and-dynamic-dispatch.md)，配套示例见 [OverloadResolutionDemo.java](../../examples/src/main/java/com/xuegucheng/javatechreview/OverloadResolutionDemo.java)、[OverloadResolutionDemoTest.java](../../examples/src/test/java/com/xuegucheng/javatechreview/OverloadResolutionDemoTest.java) 和 [examples/README.md](../../examples/README.md)。
...
泛型重写、桥接方法和名称冲突的完整规则由 [generics.md](generics.md) 统一维护；这里仅保留“重载声明必须在擦除后仍可区分”的边界。
...
- 参数是值还是引用副本：见 [parameter-passing.md](parameter-passing.md)；
- 数值宽化、窄化和二元数值提升：见 [conversions-and-numeric-precision.md](conversions-and-numeric-precision.md)；
- 方法重写、协变返回、异常和桥接：见 [inheritance-and-overriding.md](inheritance-and-overriding.md) 与 [generics.md](generics.md)；
- 动态绑定与多态：见 [polymorphism-and-dynamic-dispatch.md](polymorphism-and-dynamic-dispatch.md)；
- 可运行验证：见 [examples/README.md](../../examples/README.md)。
