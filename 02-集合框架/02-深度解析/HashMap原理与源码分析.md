...
> **本章唯一负责 HashMap 的 hash 扰动、桶定位、冲突结构、树化与 resize。** `equals/hashCode` 的语言契约由 [Java Core 对象契约](../../01-Java核心/02-深度解析/equals与hashCode契约.md)维护；本章只说明 HashMap 在桶内如何消费它。

配套资源：[HashMap 扩容图](../03-图示/HashMap/HashMap扩容.svg)、[HashMapCollisionDemo.java](../04-示例代码/src/main/java/com/xuegucheng/javatechreview/collections/HashMapCollisionDemo.java) 和 [HashMapCollisionDemoTest.java](../04-示例代码/src/test/java/com/xuegucheng/javatechreview/collections/HashMapCollisionDemoTest.java)。面试入口见 [HashMap 与 Hash 集合](../01-面试速记/HashMap与Hash集合.md)。
...
这类“先建索引再关联”的思维在实体、资源、用户和配置映射里非常常见。
...
完整运行入口统一见 [04-示例代码/README.md](../04-示例代码/README.md)；本节只保留 HashMap 契约和索引思路的短机制片段。
...
下一章 `Hash集合与equals-hashCode契约.md` 再把对象相等性、HashSet 去重和哈希集合契约单独讲透。
