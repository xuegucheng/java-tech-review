...
>
> [Java Core equality contract](../../01-Java核心/02-深度解析/equals与hashCode契约.md) 是 `==`、`Object.equals`、五项 equals 契约、hashCode 契约、继承、record、ORM 代理和可变 equality 的唯一权威来源。本章不重新定义这些规则，只解释 HashMap、HashSet 和 LinkedHashSet 如何消费它们。
...
这也是仓库中 [HashMapCollisionDemo](../04-示例代码/src/main/java/com/xuegucheng/javatechreview/collections/HashMapCollisionDemo.java) 的验证目标。
...
这里不再解释 equals 五项契约和实现模板，统一跳转 [Java equality contract](../../01-Java核心/02-深度解析/equals与hashCode契约.md)。
...
- 实体自然键和数据库 ID 的生命周期选择，见 [Java equality contract](../../01-Java核心/02-深度解析/equals与hashCode契约.md)；
...
HashSet 的结构与遍历细节见 [HashSet 与 LinkedHashSet](./HashSet与LinkedHashSet.md)；本页只负责解释它如何消费 HashMap 的 key 规则。
...
普通重复 `add` 不应被理解成一次新的插入：key 已经命中，集合大小不增加，原有 encounter order 也不因重复值重新排到尾部。Java 21 的 `SequencedSet` 首尾能力属于版本 API 增量，完整实现说明见 [HashSet 与 LinkedHashSet](./HashSet与LinkedHashSet.md)。
...
不要在本页复制 ORM 代理、继承 equality、record 数组组件或实体暂态 ID 的完整讨论。它们的唯一权威来源是 [Java equality contract](../../01-Java核心/02-深度解析/equals与hashCode契约.md)。
