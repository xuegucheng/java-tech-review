...
> **本章唯一负责 LinkedHashMap 的顺序链表、`accessOrder`、LRU 和 Java 21 `SequencedMap` 增量。** HashMap 的桶/resize 回到 [HashMap](HashMap原理与源码分析.md)，key 的 equality 契约回到 [Java Core 对象契约](../../01-Java核心/02-深度解析/equals与hashCode契约.md)。

配套资源：[LinkedHashMap 与 LRU 图](../03-图示/LinkedHashMap/LinkedHashMap与LRU.svg)、[LinkedHashMap 数据结构图](../03-图示/LinkedHashMap/LinkedHashMap数据结构.png)、[LinkedHashMapLruDemo.java](../04-示例代码/src/main/java/com/xuegucheng/javatechreview/collections/LinkedHashMapLruDemo.java) 和 [LinkedHashMapLruDemoTest.java](../04-示例代码/src/test/java/com/xuegucheng/javatechreview/collections/LinkedHashMapLruDemoTest.java)。面试入口见 [有序 Map 与 LRU](../01-面试速记/有序Map与LRU缓存.md)。
...
本章仍然只保留真正帮助理解的验证片段；完整运行入口统一见 [04-示例代码/README.md](../04-示例代码/README.md)。
